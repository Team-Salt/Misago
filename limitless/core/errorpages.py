...
from .utils import get_exception_message, is_request_to_limitless
...
def _error_page(request, code, exception=None, default_message=None):
    request.frontend_context.update({"CURRENT_LINK": "limitless:error-%s" % code})

    return render(
        request,
        "limitless/errorpages/%s.html" % code,
...
def banned(request, exception):
    ...
    request.frontend_context.update(
        {"MESSAGE": ban.get_serialized_message(), "CURRENT_LINK": "limitless:error-banned"}
    )

    return render(request, "limitless/errorpages/banned.html", {"ban": ban}, status=403)
...
def social_auth_failed(request, exception):
    ...
    return render(
        request,
        "limitless/errorpages/social.html",
        {
...
def csrf_failure(request, reason=""):
    ...
    return render(request, "limitless/errorpages/csrf_failure.html", status=403)
...
def shared_403_exception_handler(f):
    def page_decorator(request, *args, **kwargs):
        if is_request_to_limitless(request):
