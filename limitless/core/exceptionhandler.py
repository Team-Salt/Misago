...
def is_limitless_exception(exception):
...
def get_exception_handler(exception):
    for exception_type, handler in EXCEPTION_HANDLERS:
        if isinstance(exception, exception_type):
            return handler
    raise ValueError("%s is not a LimitLess exception" % exception.__class__.__name__)


def handle_limitless_exception(request, exception):
