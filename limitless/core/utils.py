...
LIMITLESS_SLUGIFY = getattr(settings, "LIMITLESS_SLUGIFY", "limitless.core.slugify.default")

slugify = import_string(LIMITLESS_SLUGIFY)
...
def is_request_to_limitless(request):
    try:
        return request._request_to_limitless
    except AttributeError:
        request._request_to_limitless = _is_request_path_under_limitless(request)
        return request._request_to_limitless


def _is_request_path_under_limitless(request):
    # We are assuming that forum_index link is root of all LimitLess links
    forum_index = reverse("limitless:index")
    path = request.path
