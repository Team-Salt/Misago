...
from ..utils import (
    ...
    is_referer_local,
    is_request_to_limitless,
    parse_iso8601_string,
    slugify,
)


class IsRequestToLimitLessTests(TestCase):
    def test_is_request_to_limitless(self):
        """
        is_request_to_limitless correctly detects requests directed at LimitLess
        """
        VALID_PATHS = ("/", "/threads/")
        INVALID_PATHS = ("", "somewhere/")

        limitless_prefix = reverse("limitless:index")

        for path in VALID_PATHS:
            request = RequestFactory().get("/")
            request.path = path
            self.assertTrue(
                is_request_to_limitless(request),
                '"%s" is not overlapped by "%s"' % (path, limitless_prefix),
            )
        ...
        for path in INVALID_PATHS:
            ...
            self.assertFalse(
                is_request_to_limitless(request),
                '"%s" is overlapped by "%s"' % (path, limitless_prefix),
            )


class SlugifyTests(TestCase):
    def test_valid_slugify_output(self):
        """LimitLess's slugify correctly slugifies string"""
        test_cases = [
            ("User", "user"),
...
PLAINTEXT_FORMAT_CASES = [
    ...
    (
        "http://limitless-project.org/login/",
        (
            "<p>"
            '<a href="http://limitless-project.org/login/">'
            "http://limitless-project.org/login/"
            "</a>"
            "</p>"
        ),
...
class CleanReturnPathTests(TestCase):
    def test_get_request(self):
        ...
        bad_request = MockRequest(
            "GET",
            {"HTTP_REFERER": "http://cookies.com", "HTTP_HOST": "limitless-project.org"},
        )
        ...
        bad_request = MockRequest(
            "GET",
            {
                "HTTP_REFERER": "https://limitless-project.org/",
                "HTTP_HOST": "limitless-project.org/",
            },
        )
        self.assertIsNone(clean_return_path(bad_request))
        ...
        bad_request = MockRequest(
            ...
            {
                "HTTP_REFERER": "https://limitless-project.org/",
                "HTTP_HOST": "limitless-project.org/assadsa/",
            },
        )
        self.assertIsNone(clean_return_path(bad_request))
        ...
        ok_request = MockRequest(
            "GET",
            {
                "HTTP_REFERER": "http://limitless-project.org/",
                "HTTP_HOST": "limitless-project.org/",
            },
        )
        ...
        ok_request = MockRequest(
            "GET",
            {
                "HTTP_REFERER": "http://limitless-project.org/login/",
                "HTTP_HOST": "limitless-project.org/",
            },
        )
