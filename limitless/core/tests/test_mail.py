...
class MailTests(TestCase):
    def test_building_mail_without_context_raises_value_error(self):
        ...
        with self.assertRaises(ValueError):
            build_mail(user, "LimitLess Test Mail", "limitless/emails/base")

    def test_building_mail_without_settings_in_context_raises_value_error(self):
        user = create_test_user("User", "user@example.com")
        with self.assertRaises(ValueError):
            build_mail(
                user, "LimitLess Test Mail", "limitless/emails/base", context={"settings": {}}
            )
    ...
    def test_mail_user(self):
        ...
        mail_user(
            user,
            "LimitLess Test Mail",
            "limitless/emails/base",
            context={"settings": settings},
        )

        self.assertEqual(mail.outbox[0].subject, "LimitLess Test Mail")

        # assert that url to user's avatar is valid
        html_body = mail.outbox[0].alternatives[0][0]
        user_avatar_url = reverse(
            "limitless:user-avatar", kwargs={"pk": user.pk, "size": 32}
        )
    ...
    def test_mail_users(self):
        ...
        for message in mail.outbox:
            if message.subject == "LimitLess Test Spam":
                spams_sent += 1
