...
class CSRFErrorViewTests(TestCase):
    def test_csrf_failure_is_handled(self):
        ...
        response = csrf_client.post(reverse("limitless:index"), data={"eric": "fish"})
...
@override_settings(ROOT_URLCONF="limitless.core.testproject.urls")
class ErrorPageViewsTests(TestCase):
    def test_banned_returns_403(self):
        """banned error page has no show-stoppers"""
        response = self.client.get(reverse("raise-limitless-banned"))
        self.assertContains(response, "limitless:error-banned", status_code=403)
    ...
    def test_permission_denied_returns_403(self):
        ...
        response = self.client.get(reverse("raise-limitless-403"))
        self.assertContains(response, "limitless:error-403", status_code=403)
    ...
    def test_page_not_found_returns_404(self):
        ...
        response = self.client.get(reverse("raise-limitless-404"))
        self.assertContains(response, "limitless:error-404", status_code=404)
        self.assertContains(response, "Page not found", status_code=404)

    def test_not_allowed_returns_405(self):
        """not allowed error page has no showstoppers"""
        response = self.client.get(reverse("raise-limitless-405"))
        self.assertContains(response, "limitless:error-405", status_code=405)
    ...
    def test_social_failed_message(self):
        """limitless-specific social auth failed error page returns 403 with message"""
    ...
    def test_social_auth_banned(self):
        """limitless-specific social auth banned error page returns 403 with ban message"""
...
@override_settings(ROOT_URLCONF="limitless.core.testproject.urlswitherrorhandlers")
class CustomErrorPagesTests(TestCase):
    def setUp(self):
        self.limitless_request = create_request(reverse("limitless:index"))
    ...
    def test_shared_403_decorator(self):
        ...
        response = mock_custom_403_error_page(self.limitless_request, PermissionDenied())
    ...
    def test_shared_404_decorator(self):
        ...
        response = mock_custom_404_error_page(self.limitless_request, Http404())
