...
class IsLimitLessExceptionTests(TestCase):
    def test_is_limitless_exception_true_for_handled_exceptions(self):
        """exceptionhandler.is_limitless_exception recognizes handled exceptions"""
        for exception in exceptionhandler.HANDLED_EXCEPTIONS:
            self.assertTrue(exceptionhandler.is_limitless_exception(exception()))

    def test_is_limitless_exception_false_for_not_handled_exceptions(self):
        """exceptionhandler.is_limitless_exception fails to recognize other exceptions"""
        for exception in INVALID_EXCEPTIONS:
            self.assertFalse(exceptionhandler.is_limitless_exception(exception()))
...
class GetExceptionHandlerTests(TestCase):
    ...
    def test_get_exception_handler_for_handled_exceptions(self):
        """Exception handler has correct handler for every LimitLess exception"""
        for exception in exceptionhandler.HANDLED_EXCEPTIONS:
