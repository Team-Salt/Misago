...
LIMITLESS_ATTACHMENT_VIEWS = ("limitless:attachment", "limitless:attachment-thumbnail")
...
def linkify_paragraphs(result):
    result["parsed_text"] = bleach.linkify(
        result["parsed_text"],
        callbacks=settings.LIMITLESS_BLEACH_CALLBACKS,
...
def clean_attachment_link(link, force_shva=False):
    ...
    if url_name in LIMITLESS_ATTACHMENT_VIEWS:
        if force_shva:
            link = "%s?shva=1" % link
