...
def list_url(admin_client):
    response = admin_client.get(reverse("limitless:admin:settings:agreements:index"))
...
def test_nav_contains_agreements_link(admin_client, list_url):
    ...
    assert_contains(response, reverse("limitless:admin:settings:agreements:index"))
...
def test_creation_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:settings:agreements:new"))
    assert response.status_code == 200


def test_form_creates_new_agreement(admin_client):
    response = admin_client.post(
        reverse("limitless:admin:settings:agreements:new"),
        {
            "type": Agreement.TYPE_TOS,
            "title": "Test TOS",
...
def test_form_creates_active_agreement(mocker, admin_client):
    set_agreement_as_active = mocker.patch(
        "limitless.legal.admin.forms.set_agreement_as_active"
    )
    response = admin_client.post(
        reverse("limitless:admin:settings:agreements:new"),
        {
...
def test_edit_form_renders(admin_client, agreement):
    response = admin_client.get(
        reverse("limitless:admin:settings:agreements:edit", kwargs={"pk": agreement.pk})
    )
    assert_contains(response, agreement.title)
...
def test_edit_form_changes_active_agreement(
    ...
    response = admin_client.post(
        reverse(
            "limitless:admin:settings:agreements:edit", kwargs={"pk": other_agreement.pk}
        ),
        data={
            "type": Agreement.TYPE_TOS,
            "is_active": "1",
...
def test_edit_form_disables_active_agreement(admin_client, active_agreement):
    response = admin_client.post(
        reverse(
            "limitless:admin:settings:agreements:edit", kwargs={"pk": active_agreement.pk}
        ),
        data={
            "type": Agreement.TYPE_TOS,
            "is_active": "0",
...
def test_agreement_can_be_deleted(admin_client, agreement):
    response = admin_client.post(
        reverse("limitless:admin:settings:agreements:delete", kwargs={"pk": agreement.pk})
    )
    assert response.status_code == 302
...
def test_active_agreement_can_be_deleted(admin_client, active_agreement):
    response = admin_client.post(
        reverse(
            "limitless:admin:settings:agreements:delete",
...
def test_agreement_can_be_set_as_active(admin_client, agreement):
    response = admin_client.post(
        reverse(
            "limitless:admin:settings:agreements:set-as-active",
...
def test_active_agreement_can_be_disabled(admin_client, active_agreement):
    response = admin_client.post(
        reverse(
            "limitless:admin:settings:agreements:disable",
            kwargs={"pk": active_agreement.pk},
        )
    )
    assert response.status_code == 302
