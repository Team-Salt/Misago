...
def test_nav_contains_menus_item(admin_client, list_url):
    ...
    assert_contains(response, reverse("limitless:admin:settings:menu-items:index"))
...
def test_creation_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:settings:menu-items:new"))
    assert response.status_code == 200


def test_form_creates_new_menu_item(admin_client):
    response = admin_client.post(
        reverse("limitless:admin:settings:menu-items:new"),
        {
            "menu": MenuItem.MENU_FOOTER,
            "title": "Test Item",
...
def test_edit_form_renders(admin_client, menu_item):
    response = admin_client.get(
        reverse("limitless:admin:settings:menu-items:edit", kwargs={"pk": menu_item.pk})
    )
    assert_contains(response, menu_item.title)
...
def test_menu_item_can_be_deleted(admin_client, menu_item):
    response = admin_client.post(
        reverse("limitless:admin:settings:menu-items:delete", kwargs={"pk": menu_item.pk})
    )
    assert response.status_code == 302
