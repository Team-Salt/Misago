...
logger = logging.getLogger("limitless.users.datadownloads")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        working_dir = settings.LIMITLESS_USER_DATA_DOWNLOADS_WORKING_DIR
        if not working_dir:
            self.stdout.write(
                "LIMITLESS_USER_DATA_DOWNLOADS_WORKING_DIR has to be set in order for "
        ...
        for data_download in chunk_queryset(queryset):
            if prepare_user_data_download(data_download, expires_in, logger):
                ...
                mail_user(
                    user,
                    subject,
                    "limitless/emails/data_download",
                    context={
