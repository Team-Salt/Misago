...
def request_reset(request):
    request.frontend_context.update(
        {"SEND_PASSWORD_RESET_API": reverse("limitless:api:send-password-form")}
    )
    return render(request, "limitless/forgottenpassword/request.html")
...
def reset_password_form(request, pk, token):
    ...
    try:
        ...
        return render(
            request,
            "limitless/forgottenpassword/error.html",
            {"message": e.args[0]},
            status=400,
        )

    api_url = reverse(
        "limitless:api:change-forgotten-password", kwargs={"pk": pk, "token": token}
    )

    request.frontend_context["CHANGE_PASSWORD_API"] = api_url
    return render(request, "limitless/forgottenpassword/form.html")
