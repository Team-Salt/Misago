...
def request_activation(request):
    request.frontend_context.update(
        {"SEND_ACTIVATION_API": reverse("limitless:api:send-activation")}
    )
    return render(request, "limitless/activation/request.html")
...
def activate_by_token(request, pk, token):
    ...
    try:
        ...
        return render(request, "limitless/activation/stopped.html", {"message": e.args[0]})
    except ActivationError as e:
        return render(
            request, "limitless/activation/error.html", {"message": e.args[0]}, status=400
        )
    ...
    return render(
        request,
        "limitless/activation/done.html",
