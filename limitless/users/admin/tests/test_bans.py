...
def admin_link(admin_client):
    response = admin_client.get(reverse("limitless:admin:users:bans:index"))
...
def test_link_is_registered_in_admin_nav(admin_client):
    response = admin_client.get(reverse("limitless:admin:users:index"))
    response = admin_client.get(response["location"])
    assert_contains(response, reverse("limitless:admin:users:bans:index"))
...
def test_ban_can_be_deleted(admin_client, ban):
    admin_client.post(reverse("limitless:admin:users:bans:delete", kwargs={"pk": ban.pk}))
...
def test_deleting_ban_invalidates_bans_cache(admin_client, ban):
    with assert_invalidates_cache(BANS_CACHE):
        admin_client.post(
            reverse("limitless:admin:users:bans:delete", kwargs={"pk": ban.pk})
        )


def test_new_ban_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:users:bans:new"))
    assert response.status_code == 200
...
def test_new_ban_can_be_created(admin_client):
    ...
    response = admin_client.post(
        reverse("limitless:admin:users:bans:new"),
        data={
...
def test_edit_ban_form_renders(admin_client, ban):
    response = admin_client.get(
        reverse("limitless:admin:users:bans:edit", kwargs={"pk": ban.pk})
    )
    assert response.status_code == 200
