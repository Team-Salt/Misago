...
class DataDownloadAdminTests(AdminTestCase):
    def test_link_registered(self):
        ...
        self.assertContains(
            response, reverse("limitless:admin:users:data-downloads:index")
        )

    def test_list_view(self):
        """data downloads list view returns 200"""
        response = self.client.get(reverse("limitless:admin:users:data-downloads:index"))
    ...
    def test_expire_action(self):
        ...
        response = self.client.post(
            reverse("limitless:admin:users:data-downloads:index"),
    ...
    def test_request_view(self):
        ...
        response = self.client.get(reverse("limitless:admin:users:data-downloads:request"))
        self.assertEqual(response.status_code, 200)

        other_user = create_test_user("OtherUser", "OtherUser@example.com")

        response = self.client.post(
            reverse("limitless:admin:users:data-downloads:request"),
            data={
