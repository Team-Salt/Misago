...
def test_link_is_registered_in_admin_nav(admin_client):
    response = admin_client.get(reverse("limitless:admin:index"))
    assert_contains(response, reverse("limitless:admin:users:index"))
...
def test_new_user_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:users:new"))
...
def test_new_user_can_be_created(admin_client):
    ...
    admin_client.post(
        reverse("limitless:admin:users:new"),
        data={
            "username": "User",
...
def test_edit_user_form_renders(admin_client, user):
    response = admin_client.get(
        reverse("limitless:admin:users:edit", kwargs={"pk": user.pk})
    )
    assert response.status_code == 200
...
def test_edit_staff_form_renders_for_staff_user(staff_client, other_staffuser):
    response = staff_client.get(
        reverse("limitless:admin:users:edit", kwargs={"pk": other_staffuser.pk})
    )
    assert response.status_code == 200
...
def test_edit_superuser_form_renders_for_staff_user(staff_client, superuser):
    response = staff_client.get(
        reverse("limitless:admin:users:edit", kwargs={"pk": superuser.pk})
    )
    assert response.status_code == 200
...
def test_edit_form_changes_user_username(admin_client, user):
    ...
    admin_client.post(
        reverse("limitless:admin:users:edit", kwargs={"pk": user.pk}), data=form_data
    )
...
def test_admin_editing_their_own_password_is_not_logged_out(admin_client, superuser):
    ...
    admin_client.post(
        reverse("limitless:admin:users:edit", kwargs={"pk": superuser.pk}), data=form_data
    )
...
def test_staff_user_cannot_promote_themselves_to_superuser(staff_client, staffuser):
    ...
    staff_client.post(
        reverse("limitless:admin:users:edit", kwargs={"pk": staffuser.pk}), data=form_data
    )
...
def test_superuser_can_degrade_other_superuser_to_staff_user(
    ...
    admin_client.post(
        reverse("limitless:admin:users:edit", kwargs={"pk": other_superuser.pk}),
        data=form_data,
    )
