...
class TestDjangoAdminUserForm(AdminTestCase):
    def setUp(self):
        ...
        self.edit_test_user_in_django_url = reverse(
            "admin:limitless_users_user_change", args=[self.test_user.pk]
        )
        self.edit_test_user_in_limitless_url = reverse(
            "limitless:admin:users:edit", args=[self.test_user.pk]
        )
    ...
    def test_limitless_admin_url_presence_in_user_edit_view(self):
        """the url to LimitLess admin is present in Django admin user edit view"""
        response = self.client.get(self.edit_test_user_in_django_url)
        self.assertContains(response, self.edit_test_user_in_limitless_url)
        edit_from_limitless_short_desc = (
            UserAdminModel.get_edit_from_limitless_url.short_description
        )
        self.assertContains(response, edit_from_limitless_short_desc)

    def test_limitless_admin_url_presence_in_user_list_view(self):
        """the url to LimitLess admin is present in Django admin user list view"""
        response = self.client.get(reverse("admin:limitless_users_user_changelist"))
        self.assertContains(response, self.edit_test_user_in_limitless_url)
