...
class OnlineTrackerMiddleware(MiddlewareMixin):
    def process_request(self, request):
        if request.user.is_authenticated:
            try:
                request._limitless_online_tracker = request.user.online_tracker
            except Online.DoesNotExist:
                tracker.start_tracking(request, request.user)
        else:
            request._limitless_online_tracker = None

    def process_response(self, request, response):
        if hasattr(request, "_limitless_online_tracker"):
            online_tracker = request._limitless_online_tracker
