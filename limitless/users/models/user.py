...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    # LimitLess stores user email in two fields:
    ...
    roles = models.ManyToManyField("limitless_acl.Role")
    ...
    class Meta:
        indexes = [
            models.Index(
                name="limitless_user_is_staff_part",
                fields=["is_staff"],
                condition=Q(is_staff=True),
            ),
            models.Index(
                name="limitless_user_requires_acti_part",
                fields=["requires_activation"],
                condition=Q(requires_activation__gt=0),
            ),
            models.Index(
                name="limitless_user_is_deleting_a_part",
    ...
    def get_absolute_url(self):
        return reverse("limitless:user", kwargs={"slug": self.slug, "pk": self.pk})
