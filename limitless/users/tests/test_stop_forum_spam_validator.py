...
def api_mock(mocker):
    return mocker.patch(
        "limitless.users.validators.requests",
        get=Mock(
            return_value=Mock(
                content=json.dumps(
...
def test_validator_logs_api_error(mocker, request_mock):
    ...
    logger_mock = mocker.patch("limitless.users.validators.logger", exception=Mock())
...
def test_validator_handles_malformed_api_response(mocker, request_mock):
    failing_api_mock = mocker.patch(
        "limitless.users.validators.requests", get=Mock(return_value=Mock(content="{}"))
    )
