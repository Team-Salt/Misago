...
class UserThreadsApiTests(ThreadsApiTestCase):
    def setUp(self):
        ...
        self.api_link = reverse("limitless:api:user-threads", kwargs={"pk": self.user.pk})

    def test_invalid_user_id(self):
        """api validates user id"""
        link = reverse("limitless:api:user-threads", kwargs={"pk": "abcd"})
        response = self.client.get(link)
        self.assertEqual(response.status_code, 404)

    def test_nonexistant_user_id(self):
        """api validates that user for id exists"""
        link = reverse("limitless:api:user-threads", kwargs={"pk": self.user.pk + 1})
        response = self.client.get(link)
        self.assertEqual(response.status_code, 404)
...
class UserPostsApiTests(ThreadsApiTestCase):
    def setUp(self):
        ...
        self.api_link = reverse("limitless:api:user-posts", kwargs={"pk": self.user.pk})

    def test_invalid_user_id(self):
        """api validates user id"""
        link = reverse("limitless:api:user-posts", kwargs={"pk": "abcd"})
        response = self.client.get(link)
        self.assertEqual(response.status_code, 404)

    def test_nonexistant_user_id(self):
        """api validates that user for id exists"""
        link = reverse("limitless:api:user-posts", kwargs={"pk": self.user.pk + 1})
        response = self.client.get(link)
        self.assertEqual(response.status_code, 404)
