...
class DenyAuthenticatedTests(UserTestCase):
    def test_success(self):
        ...
        response = self.client.post(reverse("limitless:request-activation"))
...
class DenyGuestsTests(UserTestCase):
    def test_success(self):
        ...
        response = self.client.post(reverse("limitless:options"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_ref_login(self):
        ...
        response = self.client.post("%s?ref=login" % reverse("limitless:options"))
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response["location"], reverse("limitless:index"))
