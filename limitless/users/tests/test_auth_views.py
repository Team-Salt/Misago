...
class AuthViewsTests(TestCase):
    def test_auth_views_return_302(self):
        ...
        response = self.client.get(reverse("limitless:login"))
        self.assertEqual(response.status_code, 302)

        response = self.client.post(reverse("limitless:login"))
        self.assertEqual(response.status_code, 302)

        response = self.client.get(reverse("limitless:logout"))
        self.assertEqual(response.status_code, 302)

        response = self.client.post(reverse("limitless:logout"))
        self.assertEqual(response.status_code, 302)
    ...
    def test_login_view_redirect_to(self):
        ...
        response = self.client.post(
            reverse("limitless:login"), data={"redirect_to": "/redirect/"}
        )
        ...
        response = self.client.post(
            reverse("limitless:login"), data={"redirect_to": "limitless:users"}
        )
        ...
        response = self.client.post(
            reverse("limitless:login"), data={"redirect_to": "canada goose not url!"}
        )
        ...
        response = self.client.post(
            reverse("limitless:login"), data={"redirect_to": "łelcome!"}
        )
