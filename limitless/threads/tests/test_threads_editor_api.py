...
class ThreadPostEditorApiTests(EditorApiTestCase):
    def setUp(self):
        ...
        self.api_link = reverse("limitless:api:thread-editor")
...
class ThreadReplyEditorApiTests(EditorApiTestCase):
    def setUp(self):
        ...
        self.api_link = reverse(
            "limitless:api:thread-post-editor", kwargs={"thread_pk": self.thread.pk}
        )
            ...
            for _ in range(3):
                with open(TEST_DOCUMENT_PATH, "rb") as upload:
                    response = self.client.post(
                        reverse("limitless:api:attachment-list"), data={"upload": upload}
                    )
                self.assertEqual(response.status_code, 200)
