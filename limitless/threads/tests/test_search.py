...
class SearchApiTests(AuthenticatedUserTestCase):
    def setUp(self):
        ...
        self.api_link = reverse("limitless:api:search")
...
class SearchProviderApiTests(SearchApiTests):
    def setUp(self):
        ...
        self.api_link = reverse(
            "limitless:api:search", kwargs={"search_provider": "threads"}
        )
...
def test_post_search_filters_hook_is_used_by_threads_search(
    ...
    mocker.patch(
        "limitless.threads.filtersearch.hooks.post_search_filters", [search_filter]
    )
