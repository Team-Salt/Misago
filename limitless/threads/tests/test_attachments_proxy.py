...
def test_proxy_redirects_to_404_image_for_nonexistant_attachment(db, client):
    response = client.get(
        reverse("limitless:attachment", kwargs={"pk": 1, "secret": "secret"})
    )
    assert_404(response)
...
def test_proxy_redirects_to_404_image_for_url_with_invalid_attachment_secret(
    ...
    response = client.get(
        reverse("limitless:attachment", kwargs={"pk": attachment.id, "secret": "invalid"})
    )
    assert_404(response)
...
def test_thumbnail_proxy_redirects_to_404_for_non_image_attachment(client, attachment):
    response = client.get(
        reverse(
            "limitless:attachment-thumbnail",
