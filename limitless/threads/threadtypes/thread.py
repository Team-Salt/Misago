...
class Thread(ThreadType):
    ...
    def get_category_absolute_url(self, category):
        if category.level:
            return reverse(
                "limitless:category", kwargs={"pk": category.pk, "slug": category.slug}
            )

        return reverse("limitless:threads")
    ...
    def get_thread_absolute_url(self, thread, page=1):
        ...
        return reverse("limitless:thread", kwargs={"slug": thread.slug, "pk": thread.pk})

    def get_thread_last_post_url(self, thread):
        return reverse(
            "limitless:thread-last", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_new_post_url(self, thread):
        return reverse(
            "limitless:thread-new", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_best_answer_url(self, thread):
        return reverse(
            "limitless:thread-best-answer", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_unapproved_post_url(self, thread):
        return reverse(
            "limitless:thread-unapproved", kwargs={"slug": thread.slug, "pk": thread.pk}
        )

    def get_thread_api_url(self, thread):
        return reverse("limitless:api:thread-detail", kwargs={"pk": thread.pk})

    def get_thread_editor_api_url(self, thread):
        return reverse("limitless:api:thread-post-editor", kwargs={"thread_pk": thread.pk})

    def get_thread_merge_api_url(self, thread):
        return reverse("limitless:api:thread-merge", kwargs={"pk": thread.pk})

    def get_thread_poll_api_url(self, thread):
        return reverse("limitless:api:thread-poll-list", kwargs={"thread_pk": thread.pk})

    def get_thread_posts_api_url(self, thread):
        return reverse("limitless:api:thread-post-list", kwargs={"thread_pk": thread.pk})

    def get_poll_api_url(self, poll):
        return reverse(
            "limitless:api:thread-poll-detail",
            kwargs={"thread_pk": poll.thread_id, "pk": poll.pk},
        )

    def get_poll_votes_api_url(self, poll):
        return reverse(
            "limitless:api:thread-poll-votes",
            kwargs={"thread_pk": poll.thread_id, "pk": poll.pk},
        )

    def get_post_merge_api_url(self, thread):
        return reverse("limitless:api:thread-post-merge", kwargs={"thread_pk": thread.pk})

    def get_post_move_api_url(self, thread):
        return reverse("limitless:api:thread-post-move", kwargs={"thread_pk": thread.pk})

    def get_post_split_api_url(self, thread):
        return reverse("limitless:api:thread-post-split", kwargs={"thread_pk": thread.pk})

    def get_post_absolute_url(self, post):
        return reverse(
            "limitless:thread-post",
            kwargs={"slug": post.thread.slug, "pk": post.thread.pk, "post": post.pk},
        )

    def get_post_api_url(self, post):
        return reverse(
            "limitless:api:thread-post-detail",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )

    def get_post_likes_api_url(self, post):
        return reverse(
            "limitless:api:thread-post-likes",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )

    def get_post_editor_api_url(self, post):
        return reverse(
            "limitless:api:thread-post-editor",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )

    def get_post_edits_api_url(self, post):
        return reverse(
            "limitless:api:thread-post-edits",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )

    def get_post_read_api_url(self, post):
        return reverse(
            "limitless:api:thread-post-read",
            kwargs={"thread_pk": post.thread_id, "pk": post.pk},
        )
