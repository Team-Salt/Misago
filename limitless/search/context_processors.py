...
def search_providers(request):
    ...
    try:
        ...
        # with non-limitless's anonymous user model that has no acl support
        return {}

    request.frontend_context["SEARCH_URL"] = reverse("limitless:search")
    request.frontend_context["SEARCH_API"] = reverse("limitless:api:search")
        ...
        request.frontend_context["SEARCH_PROVIDERS"].append(
            {
                ...
                "icon": provider.icon,
                "url": reverse(
                    "limitless:search", kwargs={"search_provider": provider.url}
                ),
                "api": reverse(
                    "limitless:api:search", kwargs={"search_provider": provider.url}
                ),
                "results": None,
                "time": None,
            }
