...
admin_link = reverse("limitless:admin:permissions:index")
...
def test_new_role_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:permissions:new"))
    assert response.status_code == 200
...
def test_edit_role_form_renders(admin_client, role):
    response = admin_client.get(
        reverse("limitless:admin:permissions:edit", kwargs={"pk": role.pk})
    )
    assert response.status_code == 200
...
def test_role_can_be_deleted(admin_client, role):
    admin_client.post(
        reverse("limitless:admin:permissions:delete", kwargs={"pk": role.pk})
    )
...
def test_users_with_role_view_redirects_to_admin_users_list(admin_client, role):
    response = admin_client.get(
        reverse("limitless:admin:permissions:users", kwargs={"pk": role.pk})
    )
    assert response.status_code == 302
    assert reverse("limitless:admin:users:index") in response["location"]
