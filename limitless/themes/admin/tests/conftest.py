...
def css(admin_client, theme, mock_build_theme_css):
    url = reverse("limitless:admin:themes:upload-css", kwargs={"pk": theme.pk})
...
def media(admin_client, theme):
    url = reverse("limitless:admin:themes:upload-media", kwargs={"pk": theme.pk})
...
def mock_build_single_theme_css(mocker):
    delay = mocker.Mock()
    mocker.patch(
        "limitless.themes.admin.views.build_single_theme_css", mocker.Mock(delay=delay)
    )
    return delay
...
def mock_build_theme_css(mocker):
    delay = mocker.Mock()
    mocker.patch("limitless.themes.admin.views.build_theme_css", mocker.Mock(delay=delay))
    mocker.patch(
        "limitless.themes.admin.importer.build_theme_css", mocker.Mock(delay=delay)
    )
    return delay
...
def mock_update_remote_css_size(mocker):
    delay = mocker.Mock()
    mocker.patch(
        "limitless.themes.admin.views.update_remote_css_size", mocker.Mock(delay=delay)
    )
    mocker.patch(
        "limitless.themes.admin.importer.update_remote_css_size", mocker.Mock(delay=delay)
    )
    return delay
