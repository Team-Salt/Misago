...
@pytest.fixture
def create_link():
    return reverse("limitless:admin:themes:new")


@pytest.fixture
def edit_link(theme):
    return reverse("limitless:admin:themes:edit", kwargs={"pk": theme.pk})
...
def test_moving_child_theme_under_other_theme_updates_both_themes_trees(
    ...
    child_theme = Theme.objects.create(name="Child Theme", parent=theme)
    edit_link = reverse("limitless:admin:themes:edit", kwargs={"pk": child_theme.pk})
...
def test_error_message_is_set_if_user_attempts_to_edit_default_theme(
    ...
    edit_link = reverse("limitless:admin:themes:edit", kwargs={"pk": default_theme.pk})
    response = admin_client.get(edit_link)
...
def test_error_message_is_set_if_user_attempts_to_edit_nonexisting_theme(
    ...
    edit_link = reverse("limitless:admin:themes:edit", kwargs={"pk": nonexisting_theme.pk})
    response = admin_client.get(edit_link)
    assert_has_error_message(response)
