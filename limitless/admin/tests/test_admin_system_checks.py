...
admin_link = reverse("limitless:admin:index")


def test_cache_check_passess_if_cache_can_be_read_after_setting(mocker):
    mocker.patch("limitless.admin.views.index.cache.set")
    mocker.patch("limitless.admin.views.index.cache.get", return_value="ok")
    assert check_cache() == {"is_ok": True}


def test_cache_check_passess_if_cache_cant_be_read_after_setting(mocker):
    mocker.patch("limitless.admin.views.index.cache.set")
    mocker.patch("limitless.admin.views.index.cache.get", return_value=None)
    assert check_cache() == {"is_ok": False}


def test_warning_about_disabled_cache_is_displayed_on_checks_list(mocker, admin_client):
    mocker.patch("limitless.admin.views.index.check_cache", return_value={"is_ok": False})
...
class RequestMock:
    absolute_uri = "https://limitless-project.org/somewhere/"
