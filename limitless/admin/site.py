...
class AdminSite:
    ...
    def build_nodes_dict(self):
        nodes_dict = {"limitless:admin": Node(link="limitless:admin:index")}
        ...
        while self.nodes_record:
            ...
            if iterations > 512:
                message = (
                    "LimitLess Admin hierarchy is invalid or too complex to resolve. "
                    "Nodes left: %s"
                )
    ...
    def add_node(
        ...
        *,
        ...
        if self.nodes_dict:
            raise RuntimeError(
                "LimitLess admin site has already been initialized. "
...
def join_namespace(*args):
    ...
    parts.insert(0, "limitless:admin")
    return ":".join(parts)
