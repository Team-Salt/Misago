...
from ..momentjs import LIMITLESS_ADMIN_MOMENT_LOCALES
from .auth import login


def get_protected_namespace(request):
    for namespace in settings.LIMITLESS_ADMIN_NAMESPACES:
        try:
...
def get_admin_moment_locale_url():
    return get_locale_url(
        get_language(),
        static_path_template="limitless/admin/momentjs/%s.js",
        locales=LIMITLESS_ADMIN_MOMENT_LOCALES,
    )
