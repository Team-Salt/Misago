...
def _error_page(request, code, exception=None, default_message=None):
    if not is_admin_authorized(request):
        return redirect("limitless:admin:index")

    template_pattern = "limitless/admin/errorpages/%s.html" % code
...
def _csrf_failure(request, reason=""):
    if is_admin_authorized(request):
        ...
        response = render(
            request,
            "limitless/admin/errorpages/csrf_failure_authenticated.html",
            error_page=True,
        )
    else:
        response = dj_render(request, "limitless/admin/errorpages/csrf_failure.html")
