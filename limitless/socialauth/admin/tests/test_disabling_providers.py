...
def admin_link(provider):
    return reverse(
        "limitless:admin:settings:socialauth:disable", kwargs={"pk": provider.pk}
    )
...
def test_already_disabled_provider_cant_be_disabled(admin_client, disabled_provider):
    admin_link = reverse(
        "limitless:admin:settings:socialauth:disable", kwargs={"pk": disabled_provider.pk}
    )
    admin_client.post(admin_link)
...
def test_provider_that_wasnt_setup_yet_cant_be_disabled(admin_client):
    admin_link = reverse(
        "limitless:admin:settings:socialauth:disable", kwargs={"pk": "github"}
    )
...
def test_undefined_provider_cant_be_disabled(admin_client):
    admin_link = reverse(
        "limitless:admin:settings:socialauth:disable", kwargs={"pk": "undefined"}
    )
    response = admin_client.post(admin_link)
    assert response.status_code == 302
