...
def test_form_displays_for_enabled_provider(admin_client, provider):
    response = admin_client.get(
        reverse("limitless:admin:settings:socialauth:edit", kwargs={"pk": provider.pk})
    )
...
def test_form_displays_for_disabled_provider(admin_client, disabled_provider):
    response = admin_client.get(
        reverse(
            "limitless:admin:settings:socialauth:edit", kwargs={"pk": disabled_provider.pk}
        )
    )
    assert response.status_code == 200
...
def test_form_displays_for_unset_provider(admin_client):
    response = admin_client.get(
        reverse("limitless:admin:settings:socialauth:edit", kwargs={"pk": "facebook"})
    )
    assert response.status_code == 200
...
def test_form_handles_undefined_provider(admin_client):
    response = admin_client.get(
        reverse("limitless:admin:settings:socialauth:edit", kwargs={"pk": "undefined"})
    )
...
def test_form_invalidates_providers_cache(admin_client, provider):
    with assert_invalidates_cache(SOCIALAUTH_CACHE):
        admin_client.post(
            reverse(
                "limitless:admin:settings:socialauth:edit", kwargs={"pk": provider.pk}
            ),
            {"is_active": "0"},
        )
