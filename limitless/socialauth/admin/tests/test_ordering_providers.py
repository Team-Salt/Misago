...
def test_top_provider_can_be_moved_down(admin_client, provider, other_provider):
    ...
    admin_client.post(
        reverse("limitless:admin:settings:socialauth:down", kwargs={"pk": provider.pk})
    )
...
def test_top_provider_cant_be_moved_up(admin_client, provider, other_provider):
    ...
    admin_client.post(
        reverse("limitless:admin:settings:socialauth:up", kwargs={"pk": provider.pk})
    )
...
def test_disabled_provider_cant_be_moved_down(
    ...
    admin_client.post(
        reverse(
            "limitless:admin:settings:socialauth:down", kwargs={"pk": disabled_provider.pk}
        )
    )
...
def test_disabled_provider_cant_be_moved_up(admin_client, disabled_provider, provider):
    ...
    admin_client.post(
        reverse(
            "limitless:admin:settings:socialauth:up", kwargs={"pk": disabled_provider.pk}
        )
    )
...
def test_moving_up_not_setup_provider_has_no_errors(admin_client):
    response = admin_client.post(
        reverse("limitless:admin:settings:socialauth:up", kwargs={"pk": "facebook"})
    )
...
def test_moving_down_not_setup_provider_has_no_errors(admin_client):
    response = admin_client.post(
        reverse("limitless:admin:settings:socialauth:down", kwargs={"pk": "facebook"})
    )
    assert response.status_code == 302
...
def test_moving_up_undefined_provider_has_no_errors(admin_client):
    response = admin_client.post(
        reverse("limitless:admin:settings:socialauth:up", kwargs={"pk": "undefined"})
    )
    assert response.status_code == 302
...
def test_moving_down_undefined_provider_has_no_errors(admin_client):
    response = admin_client.post(
        reverse("limitless:admin:settings:socialauth:down", kwargs={"pk": "undefined"})
    )
    assert response.status_code == 302
