...
class CategoryAdminViewsTests(CategoryAdminTestCase):
    def test_link_registered(self):
        ...
        response = self.client.get(reverse("limitless:admin:categories:index"))

        self.assertContains(response, reverse("limitless:admin:categories:index"))
    ...
    def test_new_view(self):
        ...
        response = self.client.get(reverse("limitless:admin:categories:new"))
        self.assertEqual(response.status_code, 200)

        response = self.client.post(
            reverse("limitless:admin:categories:new"),
            data={
                "name": "Test Category",
    ...
    def test_edit_view(self):
        ...
        response = self.client.get(
            reverse("limitless:admin:categories:edit", kwargs={"pk": private_threads.pk})
        )
        self.assertEqual(response.status_code, 302)

        response = self.client.get(
            reverse("limitless:admin:categories:edit", kwargs={"pk": root.pk})
        )
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.get(
            reverse("limitless:admin:categories:edit", kwargs={"pk": test_category.pk})
        )
    ...
    def test_editing_category_invalidates_acl_cache(self):
        ...
        with assert_invalidates_cache(ACL_CACHE):
            self.client.post(
                reverse(
                    "limitless:admin:categories:edit", kwargs={"pk": test_category.pk}
                ),
                data={
                    "name": "Test Category Edited",
                    "new_parent": root.pk,
    ...
    def test_move_views(self):
        ...
        response = self.client.post(
            reverse("limitless:admin:categories:up", kwargs={"pk": category_b.pk})
        )
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.post(
            reverse("limitless:admin:categories:down", kwargs={"pk": category_b.pk})
        )
        self.assertEqual(response.status_code, 302)
...
class CategoryAdminDeleteViewTests(CategoryAdminTestCase):
    ...
    def test_delete_category_move_contents(self):
        ...
        response = self.client.get(
            reverse("limitless:admin:categories:delete", kwargs={"pk": self.category_b.pk})
        )
        self.assertEqual(response.status_code, 200)

        response = self.client.post(
            reverse(
                "limitless:admin:categories:delete", kwargs={"pk": self.category_b.pk}
            ),
            data={
    ...
    def test_delete_leaf_category_and_contents(self):
        ...
        response = self.client.get(
            reverse("limitless:admin:categories:delete", kwargs={"pk": self.category_d.pk})
        )
        self.assertEqual(response.status_code, 200)

        response = self.client.post(
            reverse(
                "limitless:admin:categories:delete", kwargs={"pk": self.category_d.pk}
            ),
            data={"move_children_to": "", "move_threads_to": ""},
        )
        self.assertEqual(response.status_code, 302)
