...
class CategoryRoleAdminViewsTests(AdminTestCase):
    def test_link_registered(self):
        ...
        response = self.client.get(reverse("limitless:admin:permissions:categories:index"))

        self.assertContains(
            response, reverse("limitless:admin:permissions:categories:index")
        )
    ...
    def test_new_view(self):
        ...
        response = self.client.get(reverse("limitless:admin:permissions:categories:new"))
        self.assertEqual(response.status_code, 200)

        response = self.client.post(
            reverse("limitless:admin:permissions:categories:new"),
    ...
    def test_edit_view(self):
        ...
        response = self.client.get(
            reverse(
                "limitless:admin:permissions:categories:edit", kwargs={"pk": test_role.pk}
            )
        )
    ...
    def test_editing_role_invalidates_acl_cache(self):
        ...
        with assert_invalidates_cache(ACL_CACHE):
            self.client.post(
                reverse(
                    "limitless:admin:permissions:categories:edit",
    ...
    def test_delete_view(self):
        ...
        response = self.client.post(
            reverse(
                "limitless:admin:permissions:categories:delete",
                kwargs={"pk": test_role.pk},
            )
        )
        self.assertEqual(response.status_code, 302)

        self.client.get(reverse("limitless:admin:permissions:categories:index"))
        response = self.client.get(reverse("limitless:admin:permissions:categories:index"))
    ...
    def test_change_category_roles_view(self):
        ...
        # Create test roles
        self.client.post(
            reverse("limitless:admin:permissions:new"),
        ...
        response = self.client.get(
            reverse(
                "limitless:admin:categories:permissions", kwargs={"pk": test_category.pk}
            )
        )
        ...
        with assert_invalidates_cache(ACL_CACHE):
            self.client.post(
                reverse(
                    "limitless:admin:categories:permissions",
    ...
    def test_change_role_categories_permissions_view(self):
        ...
        response = self.client.get(
            reverse("limitless:admin:permissions:categories", kwargs={"pk": test_role.pk})
        )
        self.assertEqual(response.status_code, 302)
        ...
        with assert_invalidates_cache(ACL_CACHE):
            self.client.post(
                reverse(
                    "limitless:admin:permissions:categories", kwargs={"pk": test_role.pk}
                ),
                data={
                    ("%s-role" % category_a.pk): role_comments.pk,
                    ("%s-role" % category_b.pk): role_comments.pk,
