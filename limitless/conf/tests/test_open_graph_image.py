...
def test_default_og_image_is_used_when_none_is_set(db, client):
    response = client.get("/")
    assert_contains(response, "http://test.com/static/limitless/img/og-image.jpg")
...
def test_custom_og_image_is_used_instead_of_default_one_when_set(db, client):
    ...
    response = client.get("/")
    assert_not_contains(response, "http://test.com/media/limitless/img/og-image.jpg")
...
def test_user_avatar_can_be_used_as_og_image_on_user_profiles(client, user):
    response = client.get("%sposts/" % user.get_absolute_url())
    assert_not_contains(response, "http://test.com/static/limitless/img/og-image.jpg")
