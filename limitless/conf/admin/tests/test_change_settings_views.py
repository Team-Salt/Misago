...
def messages_mock(mocker):
    return mocker.patch("limitless.conf.admin.views.messages")
...
def test_view_handles_invalid_post_request(rf, setting):
    ...
    setting.refresh_from_db()
    assert setting.value == "LimitLess"
...
def test_analytics_settings_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:settings:analytics:index"))
    assert response.status_code == 200


def test_captcha_settings_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:settings:captcha:index"))
    assert response.status_code == 200


def test_general_settings_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:settings:general:index"))
    assert response.status_code == 200


def test_sso_settings_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:settings:sso:index"))
    assert response.status_code == 200


def test_threads_settings_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:settings:threads:index"))
    assert response.status_code == 200


def test_users_settings_form_renders(admin_client):
    response = admin_client.get(reverse("limitless:admin:settings:users:index"))
    assert response.status_code == 200
